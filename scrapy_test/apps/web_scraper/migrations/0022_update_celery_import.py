...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
      orm["djcelery.periodictask"].objects.filter(pk=2).update(
        task="scrapy_test.apps.web_scraper.services.web_scraper_tasks.run_spiders_task"
      )
      orm["djcelery.periodictask"].objects.filter(pk=3).update(
        task="scrapy_test.apps.web_scraper.services.web_scraper_tasks.run_checkers_task"
      )
