...
class Migration(DataMigration):
  ...
  def forwards(self, orm):
    orm['dynamic_scraper.scraper'].objects.filter(status='A').update(
      max_items_read=200
    )

    orm['dynamic_scraper.schedulerruntime'].objects.filter(runtime_type='C').delete()

    orm['djcelery.intervalschedule'].objects.filter(pk=1).update(every=30, period='minutes')
  ...
  complete_apps = ['dynamic_scraper', 'djcelery', 'web_scraper']
  symmetrical = True
